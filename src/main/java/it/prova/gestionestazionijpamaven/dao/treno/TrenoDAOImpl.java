...
public class TrenoDAOImpl implements TrenoDAO {
	...
	public List<Treno> findAllTreniDiUnaCitta(Citta cittaInstance) throws Exception {
		TypedQuery<Treno> query = entityManager.createQuery("select t from Treno t join t.stazioni s join s.citta c where c.id = :idCitta", Treno.class);
		return query.setParameter("idCitta", cittaInstance.getId()).getResultList();
	}
